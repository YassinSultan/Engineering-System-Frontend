...
export default function WithdrawalPermissionCard({
  organizationalUnit,
  withdrawal,
  onUpdate,
}) {
          ...
          <div>
            <h6 className="text-lg font-semibold">
              ...
              <span className="ms-2 font-normal ">
                {withdrawal.value} جنية مصري
              </span>
            </h6>
            <p className="text-sm opacity-80">
              تاريخ السماح:
              <span className="ms-2">{withdrawal.date.split("T")[0]}</span>
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div>
            <Button
              size="sm"
              variant="outline"
              onClick={() => window.open(getFileUrl(withdrawal.file), "_blank")}
            >
              <FaDownload />
            </Button>
          </div>
          <Can
            action="projects:update:withdrawalPermission"
